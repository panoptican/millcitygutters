# {{PRODUCT_NAME}} — Programmatic SEO Roadmap

> **Canonical document.** This file is the single source of truth for the programmatic-page roadmap and its phase tracker. Lives at `.seo/roadmap.md` unless `config.paths.roadmap` says otherwise. Every worktree or agent picking up programmatic SEO work on this project reads this file first.
...
**You may freshly query DataForSEO or GSC** if you need data not pre-baked into your phase (current pricing, recent SERP shifts). The GSC property string is at `gsc.site_url` in `.seo/config.json`.
...
- **Domain:** {{DOMAIN}}
- **GSC property:** {{GSC_SITE_URL}}  (from `gsc.site_url` in `.seo/config.json`)
- **Bing site:** {{BING_SITE_URL}}  (from `bing.site_url`; blank if not connected)
- **OpenSEO project:** {{OPENSEO_PROJECT_ID}}  (from `openseo.project_id`; blank if not connected)
- **Authority / playable bucket:** {{DR}} → {{KD_CEILING}}  (as of {{DR_DATE}}) — the bucket comes from GSC page-1 positions, not from authority arithmetic (`references/research-recipes.md`)
- **Stack:** {{STACK}}
...
See `~/.claude/skills/seo/references/patterns/<pattern>.md` for the full TypeScript interface per pattern. Stack-specific serialization in `~/.claude/skills/seo/references/stacks/<stack>.md`.
...
All numbers from DataForSEO and GSC (or manual sources). Re-query if a phase is executed >90 days from now.
...
> Each phase is a single deployable PR. Pre-bake everything needed; re-query DataForSEO only for fresh competitor pricing/features at execution time. Mark `in_progress` when you start, `completed` (with PR link) when merged.
...
**Scope:** see `~/.claude/skills/seo/references/lanes/technical.md` for the full audit recipe. Common Phase 0 tasks:
...
**Verification:**
- [ ] `python ~/.claude/skills/seo/scripts/tech_audit.py --domain {{DOMAIN}}` returns 0 critical findings
...
See `~/.claude/skills/seo/references/lanes/offpage.md` for the full strategy. Briefs only — a human does the sending. Track submission status here:
...
See `.seo/backlink-targets.json` for the researched target list.
...
- **Rank** — DataForSEO domain rank, backlink authority on a 0-100 or 0-1000 scale (`rank_scale`). Not interchangeable with authority scores from other tools; note the scale beside any stored number.
- **KD** — DataForSEO Keyword Difficulty, 0-100. Vendor-specific: never compare it to a KD from another tool. Often null, in which case bucket on the ranking pages' average referring domains instead.
- **Bucket** — Easy / Medium / Hard, the winnability read that replaces a KD ceiling. Your playable bucket is the hardest one where GSC shows 2+ page-1 positions.
